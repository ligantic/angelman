...
Use this runbook when opening the local Angelman registry in the built-in browser.

## Start the Registry

If the local registry is not already running, run the VS Code task **Angelman: Start debug Compose**. It starts the `runserver` service with `angelman.settings` and publishes the registry on port `18000`.
...
Open `http://<gateway>:18000/account/login` in the built-in browser. For example,
the gateway is commonly `172.19.0.1`, giving
`http://172.19.0.1:18000/account/login`.
...
```
http://<gateway>:18500/api/messages
```
...
```sh
docker compose -f docker-compose.yml -f docker-compose-debug.yml exec -T -w /app/angelman runserver python manage.py shell --settings=angelman.settings -c "from django.core.mail import send_mail; assert send_mail('smtp4dev smoke test', 'Local SMTP capture is working.', 'test@xxx.local', ['recipient@example.local']) == 1"
